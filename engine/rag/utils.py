...
def clean_deepseek_response(text: str) -> str:
    """
    Removes the <think>...</think> blocks generated by DeepSeek-R1 models
    to return only the final response.
    """
    if not isinstance(text, str):
        return str(text)
    # Remove <think> content </think> across multiple lines
...
def query_router(query: str, video_id: str, timestamp: float, chat_history: str, user_id: int | None):
    ...
    if any(keyword in query.lower() for keyword in summarization_keywords):
        ...
        response = summarizer_chain.invoke({"context": full_transcript, "question": query})
        return clean_deepseek_response(response)
    ...
    if parsed_seconds is not None:
        ...
        try:
            ...
            response = time_chain.invoke({"context": context, "question": query})
            return clean_deepseek_response(response)
    ...
    try:
        # Clean the classification output to remove <think> tags if present
        raw_classification = classifier_chain.invoke({"question": query})
        classification = clean_deepseek_response(raw_classification).strip()
    ...
    if "General" in classification:
        ...
        response = get_general_chain().invoke({"question": query})
        return clean_deepseek_response(response)
